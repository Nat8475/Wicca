    ...
    if (args[0]) {
      CommandC.findOne({ _id: args[0].toLowerCase() }, async (err, cmd) => {
        ...
        if (comando.description)
          AJUDA.addField(
            `Descrição`,
            !comando.description.length? "Não tem Descrição" : comando.description
          );
        AJUDA.addField(`Usos`, !cmd ? "Comando não registrado" : cmd.usages == 0 ? "Nenhum Uso" : cmd.usages.toString());
        AJUDA.setColor("#000000")
