      ...
      await require("mongoose")
        ...
        .toArray((err, res) => {
          ...
          const EMBED = new ClientEmbed(author)
            ...
            .setDescription(
              Exp.map((x, f) => `**\`${f + 1}.\`** **${x.user}** ( Level: ${x.level} / XP: ${x.xp} )\n**\`ID:\`**: ${x.id}`)
                .slice(0, 10)
                .join("\n\n")
            )

            .setFooter(`Sua colocação ${ranking}º lugar - ( Level: ${user.Exp.level} / XP: ${user.Exp.xp} )`, message.author.displayAvatarURL({ dynamic: true, size: 2048 }))
